<!-- Card Body -->
<div class="card-body third-form d-none">
    <h5 class="text-center">DATA PENANGGUNG JAWAB SERVER</h5>
    <h6 class="text-warning text-center fw-bold">(Penanggung Jawab Teknis)</h6>

    <!-- Input Nama -->
    <div class="mt-4 mb-4 row justify-content-center">
        <label for="nama-card2" class="col-sm-3 col-form-label fw-bold">Nama</label>
        <div class="col-sm-8">
            <input name="nm-teknis" type="text" class="p-2 form-control" id="nama-card2" />
        </div>
    </div>

    <!-- Input Nip -->
    <div class="mb-4 row justify-content-center">
        <label for="nip-2" class="col-sm-3 col-form-label fw-bold">NIP</label>
        <div class="col-sm-8">
            <input name="nip-teknis" type="text" class="p-2 form-control" id="nip-2" />
        </div>
    </div>

    <!-- Input Jabatan -->
    <div class="mb-4 row justify-content-center">
        <label for="jabatan-2" class="col-sm-3 col-form-label fw-bold">Jabatan</label>
        <div class="col-sm-8">
            <input name="jbt-teknis" type="text" class="p-2 form-control" id="jabatan-2" />
        </div>
    </div>

    <!-- Email -->
    <div class="mb-4 row justify-content-center">
        <label for="mail-2" class="col-sm-3 col-form-label fw-bold">Email</label>
        <div class="col-sm-8">
            <input name="mail-teknis" type="text" class="p-2 form-control" id="mail-2" />
        </div>
    </div>

    <!-- Input No Telp -->
    <div class="mb-4 row justify-content-center">
        <label for="telp-3" class="col-sm-3 col-form-label fw-bold">No. Telp / HP</label>
        <div class="col-sm-8">
            <input name="telp-teknis" type="text" class="p-2 form-control" id="telp-3" />
        </div>
    </div>
    
    <!-- Button Lanjut & Kembali -->
    <div class="mt-5 d-grid gap-3 col-md-5 mx-auto">
        <button class="p-2 rounded-3 btn btn-primary btn-lanjut-4" type="button">Lanjut</button>
        <button class="p-2 rounded-3 btn btn-secondary btn-kembali-2" type="button">Kembali</button>
    </div>
</div>
<!-- Akhir Card Body -->

<!-- Footer Card -->
<div class="card-footer text-center f-form3 d-none">
    4 of 6
</div>