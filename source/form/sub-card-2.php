<!-- Card Body -->
<div class="card-body second-form d-none">
    <h5 class="text-center">DATA PENANGGUNG JAWAB SERVER</h5>
    <h6 class="text-danger fw-bold text-center">(Penanggung Jawab Administratif)</h6>

    <!-- Input Nama -->
    <div class="mt-4 mb-4 row justify-content-center">
        <label for="nama-1" class="col-sm-3 col-form-label fw-bold">Nama</label>
        <div class="col-sm-8">
            <input name="nm-admin" type="text" class="p-2 form-control" id="nama-1" />
        </div>
    </div>

    <!-- Input NIP -->
    <div class="mb-4 row justify-content-center">
        <label for="nip-1" class="col-sm-3 col-form-label fw-bold">NIP</label>
        <div class="col-sm-8">
            <input name="nip-admin" type="text" class="p-2 form-control" id="nip-1" />
        </div>
    </div>

    <!-- Input Jabatan -->
    <div class="mb-4 row justify-content-center">
        <label for="jabatan" class="col-sm-3 col-form-label fw-bold">Jabatan</label>
        <div class="col-sm-8">
            <input name="jbt-admin" type="text" class="p-2 form-control" id="jabatan" />
        </div>
    </div>

    <!-- Input Email -->
    <div class="mb-4 row justify-content-center">
        <label for="mail-1" class="col-sm-3 col-form-label fw-bold">Email</label>
        <div class="col-sm-8">
            <input name="mail-admin" type="text" class="p-2 form-control" id="mail-1" />
        </div>
    </div>

    <!-- Input Telp -->
    <div class="mb-4 row justify-content-center">
        <label for="telp-2" class="col-sm-3 col-form-label fw-bold">No. Telp / HP</label>
        <div class="col-sm-8">
            <input name="telp-admin" type="text" class="p-2 form-control" id="telp-2" />
        </div>
    </div>

    <!-- Tombol Kembali & Lanjut -->
    <div class="mt-5 d-grid gap-3 col-md-5 mx-auto">
        <button class="p-2 rounded-3 btn btn-primary btn-lanjut-3" type="button">Lanjut</button>
        <button class="p-2 rounded-3 btn btn-secondary btn-kembali-1" type="button">Kembali</button>
    </div>
</div>
<!-- Akhir Card Body -->

<!-- Footer Card -->
<div class="card-footer text-center f-form2 d-none">
    3 of 6
</div>